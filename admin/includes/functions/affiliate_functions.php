...
function affiliate_delete ($affiliate_id) {
  ...
  if ($affiliate = vam_db_fetch_array($affiliate_query)) {
    if ($affiliate['affiliate_root'] == $affiliate_id) {
      ...
      $affiliate_child_query = vam_db_query("
      ...
      while ($affiliate_child = vam_db_fetch_array($affiliate_child_query)) {
        ...
        vam_db_query ("UPDATE  " . TABLE_AFFILIATE . " SET affiliate_lft = affiliate_lft - " . $substract . " WHERE  affiliate_root = " . $affiliate_child['affiliate_id']);
