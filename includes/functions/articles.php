...
// TABLES: topics, topic_descriptions
  function vam_get_topics($topics_array = '', $parent_id = '0', $indent = '') {
    global $languages_id;
    ...
    while ($topics = vam_db_fetch_array($topics_query,true)) {
      ...
      if ($topics['topics_id'] != $parent_id) {
        $topics_array = vam_get_topics($topics_array, $topics['topics_id'], $indent . '&nbsp;&nbsp;');
      }
    }
  ...
  function vam_get_parent_topics(&$topics, $topics_id) {
    ...
    while ($parent_topics = vam_db_fetch_array($parent_topics_query,true)) {
      ...
      if ($parent_topics['parent_id'] != $topics_id) {
        vam_get_parent_topics($topics, $parent_topics['parent_id']);
      }
    }
  }
...
// TABLES: articles
  function vam_get_articles_name($article_id, $language = '') {
    global $languages_id;
...
// Cache the articles box
  function vam_cache_topics_box($auto_expire = false, $refresh = false) {
...
// Cache the authors box
  function vam_cache_authors_box($auto_expire = false, $refresh = false) {
    global $_GET, $language;

    $authors_id = '';
    if (isset($_GET['authors_id']) && vam_not_null($_GET['authors_id'])) {
