...
// class constructor
    function payonlinesystem() {
      ...
      $this->icon = DIR_WS_ICONS . 'payonlinesystem.png';
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PAYONLINESYSTEM_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYONLINESYSTEM_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_payonline_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_payonline_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $_SESSION['cart_payonline_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PAYONLINESYSTEM_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
                               ...
                               $hash = md5('MerchantId='.MODULE_PAYMENT_PAYONLINESYSTEM_ID.'&OrderId='.substr($_SESSION['cart_payonline_id'], strpos($_SESSION['cart_payonline_id'], '-')+1).'&Amount='.$order_sum.'&Currency=RUB&PrivateSecurityKey='.MODULE_PAYMENT_PAYONLINESYSTEM_SECRET_KEY);

      $process_button_string = vam_draw_hidden_field('OrderId', substr($_SESSION['cart_payonline_id'], strpos($_SESSION['cart_payonline_id'], '-')+1)) .
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PAYONLINESYSTEM_STATUS'");
