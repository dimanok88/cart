<?php
/* -----------------------------------------------------------------------------------------
   $Id: wallet_one.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function wallet_one() {
      ...
      $this->title = MODULE_PAYMENT_WALLET_ONE_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_WALLET_ONE_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_WALLET_ONE_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'w1.png';
      $this->sort_order = MODULE_PAYMENT_WALLET_ONE_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_WALLET_ONE_STATUS == 'True') ? true : false);

        $this->form_action_url = 'https://merchant.w1.ru/checkout/default.aspx';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_WALLET_ONE_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_WALLET_ONE_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_wallet_id'])) {
        $order_id = substr($_SESSION['cart_wallet_id'], strpos($_SESSION['cart_wallet_id'], '-')+1);
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_wallet_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_wallet_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $_SESSION['cart_wallet_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_WALLET_ONE_TEXT_DESCRIPTION);
    }
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_WALLET_ONE_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_WALLET_ONE_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WALLET_ONE_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WALLET_ONE_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WALLET_ONE_SORT_ORDER', '0', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_WALLET_ONE_ZONE', '0', '6', '5', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WALLET_ONE_SECRET_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_WALLET_ONE_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_WALLET_ONE_STATUS', 'MODULE_PAYMENT_WALLET_ONE_ALLOWED', 'MODULE_PAYMENT_WALLET_ONE_ID', 'MODULE_PAYMENT_WALLET_ONE_SORT_ORDER', 'MODULE_PAYMENT_WALLET_ONE_ZONE', 'MODULE_PAYMENT_WALLET_ONE_SECRET_KEY', 'MODULE_PAYMENT_WALLET_ONE_ORDER_STATUS_ID');
    }
