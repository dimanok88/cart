...
// class constructor
    function onpay() {
      ...
      $this->public_title = MODULE_PAYMENT_ONPAY_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_ONPAY_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'onpay.gif';
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_ONPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_ONPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_onpay_id'])) {
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
      ...
      return array('title' => MODULE_PAYMENT_ONPAY_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = vam_draw_hidden_field('pay_mode', 'fix') .
                               ...
                               vam_draw_hidden_field('currency', 'RUR') . 
                               vam_draw_hidden_field('convert', 'yes') . 
                               vam_draw_hidden_field('url_success', vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) . 
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ONPAY_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_ONPAY_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_ID', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_ONPAY_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ONPAY_SECRET_KEY', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_ONPAY_ORDER_STATUS_ID', '0', '6', '10', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_ONPAY_STATUS', 'MODULE_PAYMENT_ONPAY_ALLOWED', 'MODULE_PAYMENT_ONPAY_ID', 'MODULE_PAYMENT_ONPAY_SORT_ORDER', 'MODULE_PAYMENT_ONPAY_ZONE', 'MODULE_PAYMENT_ONPAY_SECRET_KEY', 'MODULE_PAYMENT_ONPAY_ORDER_STATUS_ID');
    }
