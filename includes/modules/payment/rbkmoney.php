...
// class constructor
    function rbkmoney() {
      ...
      $this->code = 'rbkmoney';
      $this->title = MODULE_PAYMENT_RBKMONEY_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_RBKMONEY_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_RBKMONEY_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'rbkmoney.png';
      $this->sort_order = MODULE_PAYMENT_RBKMONEY_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_RBKMONEY_STATUS == 'True') ? true : false);
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_RBKMONEY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_RBKMONEY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_rbkmoney_id'])) {
        $order_id = substr($_SESSION['cart_rbkmoney_id'], strpos($_SESSION['cart_rbkmoney_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_rbkmoney_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_rbkmoney_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_rbkmoney_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $_SESSION['cart_rbkmoney_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_RBKMONEY_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = vam_draw_hidden_field('eshopId', MODULE_PAYMENT_RBKMONEY_SHOP_ID) .
                               vam_draw_hidden_field('orderId', substr($_SESSION['cart_rbkmoney_id'], strpos($_SESSION['cart_rbkmoney_id'], '-')+1)) .
                               vam_draw_hidden_field('serviceName', substr($_SESSION['cart_rbkmoney_id'], strpos($_SESSION['cart_rbkmoney_id'], '-')+1)) .
                               vam_draw_hidden_field('recipientAmount', $order_sum) .
                               vam_draw_hidden_field('recipientCurrency', 'RUR') .
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_RBKMONEY_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_RBKMONEY_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RBKMONEY_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RBKMONEY_SHOP_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RBKMONEY_SORT_ORDER', '0', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_RBKMONEY_ZONE', '0', '6', '5', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_RBKMONEY_SECRET_KEY', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_RBKMONEY_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_RBKMONEY_STATUS', 'MODULE_PAYMENT_RBKMONEY_ALLOWED', 'MODULE_PAYMENT_RBKMONEY_SHOP_ID', 'MODULE_PAYMENT_RBKMONEY_SORT_ORDER', 'MODULE_PAYMENT_RBKMONEY_ZONE', 'MODULE_PAYMENT_RBKMONEY_SECRET_KEY', 'MODULE_PAYMENT_RBKMONEY_ORDER_STATUS_ID');
    }
