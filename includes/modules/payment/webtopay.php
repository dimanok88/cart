<?php
/* -----------------------------------------------------------------------------------------
   $Id: webtopay.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function webtopay() {
      ...
      $this->public_title = MODULE_PAYMENT_WEBTOPAY_TEXT_PUBLIC_TITLE;
      ...
      if ((int)MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID > 0) {
        $this->order_status = MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID;
      }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_WEBTOPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_WEBTOPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_webtopay_id'])) {
        $order_id = substr($_SESSION['cart_webtopay_id'], strpos($_SESSION['cart_webtopay_id'], '-')+1);
    ...
    function confirmation() {
      global $cartID, $cart_webtopay_id, $customer_id, $languages_id, $order, $order_total_modules;
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_webtopay_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_webtopay_id, 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
if ($_SERVER["HTTP_X_FORWARDED_FOR"]) {
	...
	$customers_ip = $_SERVER["REMOTE_ADDR"];
          ...
          $cart_webtopay_id = $cartID . '-' . $insert_id;
          $_SESSION['cart_webtopay_id'] = $cart_webtopay_id;
        }
      }

      return array('title' => MODULE_PAYMENT_WEBTOPAY_TEXT_DESCRIPTION);
    }

    function process_button() {
      global $customer_id, $order, $sendto, $vamPrice, $currencies, $cart_webtopay_id, $shipping;

      $process_button_string = '';

      $process_button_string = vam_draw_hidden_field('MerchantID', MODULE_PAYMENT_WEBTOPAY_ID) .
                               vam_draw_hidden_field('OrderID', substr($cart_webtopay_id, strpos($cart_webtopay_id, '-')+1)) .
                               vam_draw_hidden_field('Lang', 'RUS') .
                               vam_draw_hidden_field('Amount', number_format($vamPrice->CalculateCurrEx($order->info['total'],'USD'),2)*100) .
                               vam_draw_hidden_field('Currency', 'USD') .
                               vam_draw_hidden_field('AcceptURL', vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) .
                               vam_draw_hidden_field('CancelURL', vam_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL')) .
                               vam_draw_hidden_field('CallbackURL', vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) .
                               vam_draw_hidden_field('Payment', 'hanza2') .
                               vam_draw_hidden_field('Country', $order->customer['country']['iso_code_2']) .
                               vam_draw_hidden_field('PayText', 'Заказ номер: ' . substr($cart_webtopay_id, strpos($cart_webtopay_id, '-')+1) . ', покупатель номер: ' . $_SESSION['customer_id']);
    ...
    function before_process() {
      ...
      if (vam_db_num_rows($check_query)) {
        ...
        if ($check['orders_status'] == MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID) {
          $sql_data_array = array('orders_id' => $order_id,
                                  'orders_status_id' => MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID,
                                  'date_added' => 'now()',
                                  'customer_notified' => '0',
                                  'comments' => '');
      ...
      $sql_data_array = array('orders_id' => $order_id,
                              'orders_status_id' => (MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID),
                              'date_added' => 'now()',
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_WEBTOPAY_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ID', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_WEBTOPAY_SORT_ORDER', '0', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ZONE', '0', '6', '7', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID', '0', '6', '8', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID', '0', '6', '9', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_WEBTOPAY_STATUS', 'MODULE_PAYMENT_WEBTOPAY_ALLOWED', 'MODULE_PAYMENT_WEBTOPAY_ID', 'MODULE_PAYMENT_WEBTOPAY_SORT_ORDER', 'MODULE_PAYMENT_WEBTOPAY_ZONE', 'MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID');
    }
