...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_QIWI_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_QIWI_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
...
// Выписываем qiwi счёт для покупателя
        ...
        require_once(DIR_WS_CLASSES . 'nusoap/nusoap.php');
			...
			'comment' => substr($_SESSION['cart_qiwi_id'], strpos($_SESSION['cart_qiwi_id'], '-')+1),
			'txn' => substr($_SESSION['cart_qiwi_id'], strpos($_SESSION['cart_qiwi_id'], '-')+1),
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_QIWI_STATUS'");
